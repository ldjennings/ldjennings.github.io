...
While this was a robotics course, all of my team members were also computer science majors, so we decided to go a bit crazy with the project. Because we went overboard with the code base, including a lot of debug statements, we ran into memory limitations with the Atmega board. So, our team decided to do all of our higher level logic on the ESP-32, and communicate to the Atmega via serial. A member of our team developed a small RPC library to do this. 
